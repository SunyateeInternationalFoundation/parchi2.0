...
const Template = forwardRef((props, ref) => {
  ...
  const ModifiedPurchaseData = {
    ...purchaseData,
    items: purchaseData.products.map((item) => {
  ...
  const totalTaxableAmount = ModifiedPurchaseData.items.reduce(
    (sum, product) => sum + product.netAmount * product.quantity,
    0
  );

  const totalSgstAmount_2_5 = ModifiedPurchaseData.items.reduce(
    (sum, product) =>
      product.sgst === 2.5 ? sum + product.sgstAmount * product.quantity : sum,
    0
  );

  const totalCgstAmount_2_5 = ModifiedPurchaseData.items.reduce(
    (sum, product) =>
      product.cgst === 2.5 ? sum + product.cgstAmount * product.quantity : sum,
    0
  );

  const totalSgstAmount_6 = ModifiedPurchaseData.items.reduce(
    (sum, product) =>
      product.sgst === 6 ? sum + product.sgstAmount * product.quantity : sum,
    0
  );

  const totalCgstAmount_6 = ModifiedPurchaseData.items.reduce(
    (sum, product) =>
      product.cgst === 6 ? sum + product.cgstAmount * product.quantity : sum,
    0
  );

  const totalSgstAmount_9 = ModifiedPurchaseData.items.reduce(
    (sum, product) =>
      product.sgst === 9 ? sum + product.sgstAmount * product.quantity : sum,
    0
  );

  const totalCgstAmount_9 = ModifiedPurchaseData.items.reduce(
    (sum, product) =>
    ...
    >
      <div className="flex justify-between">
        <div>
          <div className="text-lg font-bold text-start">
            {ModifiedPurchaseData?.createdBy?.name}
          </div>
          <div>Mobile : {ModifiedPurchaseData?.createdBy?.phoneNo}</div>
        </div>
        <div>
      ...
      <div className="flex justify-between text-start">
        <div className="mt-2 text-start">
          ...
          <div>{ModifiedPurchaseData.vendorDetails.name}</div>
          <div>Ph : {ModifiedPurchaseData.vendorDetails.phone}</div>
        </div>
        <div>
          <div>
            <strong> Purchase# :</strong>{" "}
            {ModifiedPurchaseData.purchaseNo}
          </div>
          <div>
            <strong>Purchase Date :</strong>{" "}
            {DateFormate(ModifiedPurchaseData.date)}
          </div>
      ...
      <div className="w-full">
        <table className="w-full mt-5 text-sm">
          ...
          <tbody>
            {ModifiedPurchaseData.items.map((item, index) => (
              <tr key={index}>
      ...
      <div className="mt-4 text-end">
        {ModifiedPurchaseData.shippingCharges > 0 && (
          <div>
            Delivery/Shipping Charges :
            <span className="ml-5">
              {" "}
              {ModifiedPurchaseData.shippingCharges}
            </span>
          </div>
        )}
        {ModifiedPurchaseData.packagingCharges > 0 && (
          <div>
            Packaging Charges :
            <span className="ml-5">
              {" "}
              {ModifiedPurchaseData.packagingCharges}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 mb-3 text-end">
        {ModifiedPurchaseData.tcs.isTcsApplicable && (
          <div>
            TCS :
            <span className="ml-5">
              {ModifiedPurchaseData.tcs.tcs_amount.toFixed(2)}
            </span>
          </div>
        )}
        {ModifiedPurchaseData.tds.isTdsApplicable && (
          <div>
            TDS :
            <span className="ml-5">
              {ModifiedPurchaseData.tds.tds_amount.toFixed(2)}
            </span>
          </div>
        )}
        {totalCgstAmount_9 > 0 && (
      ...
      <div className="text-end font-bold">
        <h3>Total : {+ModifiedPurchaseData.total?.toFixed(2)}</h3>
      </div>
