...
function ProFormaView() {
  ...
  const fetchProForma = async () => {
    try {
      ...
      const { customerDetails, proFormaNo, ...resData } = (
        await getDoc(proFormaRef)
      ).data();
      const proFormasData = {
        id,
        ...resData,
      ...
      if (proFormasData.book?.bookRef) {
        const bankData = (await getDoc(proFormasData.book.bookRef)).data();
  ...
  return (
    <div className="px-5 pb-5 bg-gray-100" style={{ width: "100%" }}>
      ...
      <div className="w-full">
        {activeTab === "ProForma" && (
          <div>
            <ProForma proForma={proForma} bankDetails={bankDetails} />
          </div>
        )}
        {/* {activeTab === "Returns" && (
          <div>
