...
const Template = forwardRef((props, ref) => {
  ...
  const ModifiedProFormaData = {
    ...proFormaData,
    items: proFormaData.products.map((item) => {
  ...
  const totalTaxableAmount = ModifiedProFormaData.items.reduce(
    (sum, product) => sum + product.netAmount * product.quantity,
    0
  );

  const totalSgstAmount_2_5 = ModifiedProFormaData.items.reduce(
    (sum, product) =>
      product.sgst === 2.5 ? sum + product.sgstAmount * product.quantity : sum,
    0
  );

  const totalCgstAmount_2_5 = ModifiedProFormaData.items.reduce(
    (sum, product) =>
      product.cgst === 2.5 ? sum + product.cgstAmount * product.quantity : sum,
    0
  );

  const totalSgstAmount_6 = ModifiedProFormaData.items.reduce(
    (sum, product) =>
      product.sgst === 6 ? sum + product.sgstAmount * product.quantity : sum,
    0
  );

  const totalCgstAmount_6 = ModifiedProFormaData.items.reduce(
    (sum, product) =>
      product.cgst === 6 ? sum + product.cgstAmount * product.quantity : sum,
    0
  );

  const totalSgstAmount_9 = ModifiedProFormaData.items.reduce(
    (sum, product) =>
      product.sgst === 9 ? sum + product.sgstAmount * product.quantity : sum,
    0
  );

  const totalCgstAmount_9 = ModifiedProFormaData.items.reduce(
    (sum, product) =>
    ...
    >
      <div className="flex justify-between">
        <div>
          <div className="text-lg font-bold text-start">
            {ModifiedProFormaData?.createdBy?.name}
          </div>
          <div>Mobile : {ModifiedProFormaData?.createdBy?.phoneNo}</div>
        </div>
        <div>
      ...
      <div className="flex justify-between text-start">
        <div className="mt-2 text-start">
          ...
          <div>{ModifiedProFormaData.customerDetails.name}</div>
          <div>Ph : {ModifiedProFormaData.customerDetails.phone}</div>
        </div>
        <div>
          <div>
            <strong>ProFormaInvoice # :</strong>{" "}
            {ModifiedProFormaData.proFormaNo}
          </div>
          <div>
            <strong>ProFormaInvoice Date :</strong>{" "}
            {DateFormate(ModifiedProFormaData.date)}
          </div>
      ...
      <div className="w-full">
        <table className="w-full mt-5 text-sm">
          ...
          <tbody>
            {ModifiedProFormaData.items.map((item, index) => (
              <tr key={index}>
      ...
      <div className="mt-4 text-end">
        {ModifiedProFormaData.shippingCharges > 0 && (
          <div>
            Delivery/Shipping Charges :
            <span className="ml-5">
              {" "}
              {ModifiedProFormaData.shippingCharges}
            </span>
          </div>
        )}
        {ModifiedProFormaData.packagingCharges > 0 && (
          <div>
            Packaging Charges :
            <span className="ml-5">
              {" "}
              {ModifiedProFormaData.packagingCharges}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 mb-3 text-end">
        {ModifiedProFormaData.tcs.isTcsApplicable && (
          <div>
            TCS :
            <span className="ml-5">
              {ModifiedProFormaData.tcs.tcs_amount.toFixed(2)}
            </span>
          </div>
        )}
        {ModifiedProFormaData.tds.isTdsApplicable && (
          <div>
            TDS :
            <span className="ml-5">
              {ModifiedProFormaData.tds.tds_amount.toFixed(2)}
            </span>
          </div>
        )}
        {totalCgstAmount_9 > 0 && (
      ...
      <div className="text-end font-bold">
        <h3>Total : {+ModifiedProFormaData.total?.toFixed(2)}</h3>
      </div>
